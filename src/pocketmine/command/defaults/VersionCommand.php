...
class VersionCommand extends VanillaCommand{
	...
	public function execute(CommandSender $sender, $currentAlias, array $args){
		...
		if(\count($args) === 0){
		$sender->sendMessage("This server is running " . $sender->getServer()->getName() . " v" . $sender->getServer()->getPocketMineVersion() . " #" . $sender->getServer()->getSernyuBuild() . " 「 " . $sender->getServer()->getCodename() . " 」implementing API version " . $sender->getServer()->getApiVersion() . " for Minecraft: PE " . $sender->getServer()->getVersion() . " (protocol version " . Info::CURRENT_PROTOCOL . ")");
		//	$sender->sendMessage(new TranslationContainer("pocketmine.server.info.extended", [
			//	$sender->getServer()->getName(),
			//	$sender->getServer()->getPocketMineVersion(),
			//	$sender->getServer()->getSernyuBuild(),
