 ...
 *                        |___/       
 * 
 * Sernyu is a thrid-party build for Minecraft: PE Servers
 *
...
namespace pocketmine {
	...
	const CODENAME = "[Arrival]";
	const MINECRAFT_VERSION = "v0.14.x.x alpha";
	...
	if(\php_sapi_name() !== "cli"){
		$logger->critical("You must run Sernyu using the CLI.");
		++$errors;
	}
	...
	if(\extension_loaded("pocketmine")){
		if(\version_compare(\phpversion("pocketmine"), "0.0.1") < 0){
			$logger->critical("You have the native Sernyu extension, but your version is lower than 0.0.1.");
			++$errors;
		}elseif(\version_compare(\phpversion("pocketmine"), "0.0.4") > 0){
			$logger->critical("You have the native Sernyu extension, but your version is higher than 0.0.4.");
			++$errors;
		}
	}
	...
	if(\Phar::running(\true) === ""){
		$logger->warning("Non-packaged Sernyu installation detected, do not use on production.");
	}
